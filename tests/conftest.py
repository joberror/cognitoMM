...
hydrogram installs the uvloop event-loop policy at import time
(hydrogram/__init__.py calls asyncio.set_event_loop_policy(uvloop.EventLoopPolicy())).
Under pytest-asyncio's per-test event loops this intermittently raises
...
def _no_uvloop_policy(policy):
    """Refuse uvloop event-loop policies (installed e.g. by hydrogram)."""
