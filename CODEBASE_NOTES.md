...
- **CI:** `.github/workflows/static-checks.yml` runs on push/PR — `reexport-check` job: `python scripts/check_reexports.py` (stdlib-only, fails on re-export chains / broken / unknown-module imports / duplicate definitions); `tests` job: `pip install -r requirements.txt` + `python -m pytest tests/ -q` (full suite; no live MongoDB/Telegram needed — tests use fakes/mocks)
- **Local gates:** `make check` (static import check), `make test` (pytest), `make install-hooks` (installs the pre-commit hook via `git config core.hooksPath .githooks`; hook runs the static check when `features/*.py`/`main.py` are staged — see `.githooks/pre-commit`)
