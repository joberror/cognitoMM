...
2. **Interactive Prompts**: Bot guides user through the following steps:
   - **Step 1**: Content Type (Movie or Series)
   - **Step 2**: Title/Name
   - **Step 3**: Release Year
   - **Step 4**: IMDB Link (optional)
3. **Validation**: Bot validates all inputs and checks for duplicates
4. **Confirmation**: User receives confirmation with queue position
...
- **IMDB Link**: Optional, but validated if provided
  - Accepted formats:
